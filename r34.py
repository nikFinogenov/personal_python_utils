...
async def rcoin(ctx):
    ...
    if side == 50 or side > 50:
        ...
        embed.set_image(url=rdl('blowjob', random.randint(1, 100)))
        embed.set_footer(text='')
        await ctx.send(embed=embed)
    elif side < 50:
        embed = discord.Embed(title=f'NSFW Coinflip: Tails', color=ctx.author.color)
        embed.set_author(name=f'{ctx.author.display_name} - NSFW Coinflip', icon_url=f'{ctx.author.avatar.url}')
        embed.set_image(url=rdl('big_ass', random.randint(1, 100)))
        embed.set_footer(text='')
...
async def help(ctx):
    ...
    embed.add_field(name="'d6 [dice]'", value="Rolls (a/multiple) 6 sided die. Change [dice] to add several.",
                    inline=False)
    embed.add_field(name="'d8 [dice]'",
                    value="Rolls (a/multiple) 8 sided die, change your [dice] argument to add several.", inline=False)
    embed.add_field(name="'d10 [dice]'",
                    value="Rolls (a/multiple) 10 sided die, change your [dice] argument to add several.", inline=False)
    embed.add_field(name="'d12 [dice]'",
                    value="Rolls (a/multiple) 12 sided die, change your [dice] argument to add several.", inline=False)
    embed.add_field(name="'dc <sides> [dice]'",
                    value="Rolls a custom-sided die, change your <sides> argument to set sides, and your [dice] argument to add more dice.",
                    inline=False)
    ...
    embed.add_field(name="'fcoin'", value="Flips a coin and posts a nsfw furry image based on what you get.",
                    inline=False)
    ...
    embed.add_field(name="'suggest'",
                    value="Sends a link to the github to suggest features and improvements aswell as make bug reports.")
    embed.set_footer(text="",
                     icon_url='https://cdn.discordapp.com/avatars/268211297332625428/e5e43e26d4749c96b48a9465ff564ed2.png?size=128')
    await ctx.send(embed=embed)
...
async def suggest(ctx):
    await ctx.channel.send(
        f'***If you have a suggestion, make an issue on the github repo: https://github.com/whitetrashyt/r34discordbot/issues***')
